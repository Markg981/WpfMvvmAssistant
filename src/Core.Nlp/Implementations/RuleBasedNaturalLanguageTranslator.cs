...
public class RuleBasedNaturalLanguageTranslator : INaturalLanguageQueryTranslator
{
    ...
    private class QueryParser
    {
        ...
        // Extracted static regex patterns into compiled Regex instances to avoid regex recompilation
        // overhead during FindConditions and FindLimit loop operations.
        private static readonly Regex DaysPattern = new(@"last\s+(\d+)\s+days", RegexOptions.Compiled);
        private static readonly Regex TopPattern = new(@"top\s+(\d+)", RegexOptions.Compiled);
        private static readonly Regex FirstPattern = new(@"first\s+(\d+)", RegexOptions.Compiled);
        ...
        private List<QueryCondition> FindConditions(TableInfo table)
        {
            ...
            foreach (var column in table.Columns)
            {
                ...
                if (columnName.Contains("date"))
                {
                    ...
                    {
                        var daysMatch = DaysPattern.Match(_input);
