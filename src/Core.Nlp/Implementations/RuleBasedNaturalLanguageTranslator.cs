...
public class RuleBasedNaturalLanguageTranslator : INaturalLanguageQueryTranslator
{
    ...
    private class QueryParser
    {
        ...
        // Compiling these frequently used static regex patterns once per application domain
        // avoids the ~90% overhead of dynamic regex compilation during every query parsing operation.
        private static readonly Regex DaysRegex = new(@"last\s+(\d+)\s+days", RegexOptions.Compiled);
        ...
        private List<QueryCondition> FindConditions(TableInfo table)
        {
            ...
            foreach (var column in table.Columns)
            {
                ...
                if (columnName.Contains("date"))
                {
                    ...
                    {
                        var daysMatch = DaysRegex.Match(_input);
