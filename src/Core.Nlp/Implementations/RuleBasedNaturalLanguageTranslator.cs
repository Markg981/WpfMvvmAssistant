...
public class RuleBasedNaturalLanguageTranslator : INaturalLanguageQueryTranslator
{
    ...
    {
        // ⚡ BOLT OPTIMIZATION:
        // Using static RegexOptions.Compiled for static patterns used frequently
        // This avoids compilation overhead on every query.
        // We leave dynamic patterns (like the ones built inside FindConditions)
        // uncompiled as standard Regex.Match to avoid the severe memory leak and compilation
        // overhead anti-pattern associated with RegexOptions.Compiled in loops.
        private static readonly Regex LastDaysRegex = new Regex(@"last\s+(\d+)\s+days", RegexOptions.Compiled);
