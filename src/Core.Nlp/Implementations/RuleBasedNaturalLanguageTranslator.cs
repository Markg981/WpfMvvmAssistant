...
public class RuleBasedNaturalLanguageTranslator : INaturalLanguageQueryTranslator
{
    ...
    private class QueryParser
    {
        ...
        // Bolt Optimization: Compile static regex patterns that are used frequently.
        // We only compile static patterns (like "top", "first", "last X days") and deliberately
        // keep dynamic patterns (like those injecting variable `columnName`) uncompiled using Regex.Match
        // to avoid regex compilation memory leaks and avoid severe performance anti-patterns.
