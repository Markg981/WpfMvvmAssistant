...
public class RuleBasedNaturalLanguageTranslator : INaturalLanguageQueryTranslator
{
    ...
    {
        private static readonly Regex _topPattern = new Regex(@"top\s+(\d+)", RegexOptions.Compiled);
        private static readonly Regex _firstPattern = new Regex(@"first\s+(\d+)", RegexOptions.Compiled);
        private static readonly Regex _lastDaysPattern = new Regex(@"last\s+(\d+)\s+days", RegexOptions.Compiled);
